...
public class MazeGridPanel extends JPanel{
	...
	public void solveMaze() {
        ...
        while (!stack.isEmpty()) {
			...
			if (!current.northWall && !visited(current.row - 1, current.col))  {//try north
				stack.push(maze[current.row-1][current.col]);
				current.setBackground(Color.BLUE);
			}else if(!current.southWall && !visited(current.row + 1, current.col)){//try south
				stack.push(maze[current.row+1][current.col]);
				current.setBackground(Color.BLUE);
			}else if(!current.eastWall && !visited(current.row, current.col + 1)){ //try east
